# -*- coding: utf-8 -*-
"""Copy of Untitled58.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZZX1dmy-iEkUG90xnu6w0i3k72toSUYt
"""
...
# Save for LaTeX
# -----------------------------
plt.savefig("qir_gap_heatmap.pdf")   # ✅ vector (BEST)
...
# Install pylatexenc if missing (required for MatplotlibDrawer)
...
# ------------------------------------------------
# 1️⃣ FEATURE SUPERPOSITION (Global Image Encoding)
# ------------------------------------------------
for q in range(n):
    qc_logic.h(q)  # put all qubits into superposition

# ------------------------------------------------
# 2️⃣ ADAPTIVE FEATURE WEIGHTING (Intensity Encoding)
# ------------------------------------------------
for q in range(n):
    qc_logic.ry(0.8, q)  # gamma-style nonlinear weighting (representative)

# ------------------------------------------------
# 3️⃣ HIERARCHICAL ENTANGLEMENT (Block Correlation)
# ------------------------------------------------
for q in range(n - 1):
    qc_logic.cx(q, q + 1)
...
# ------------------------------------------------
# 4️⃣ FEATURE REFINEMENT
# ------------------------------------------------
for q in range(n):
    qc_logic.h(q)

# ------------------------------------------------
# 5️⃣ MEASUREMENT (Image Reconstruction Stage)
# ------------------------------------------------
...
# 1️⃣ Quantum Image Flipping (Horizontal)
def quantum_flip(image):
    return np.fliplr(image)

# 2️⃣ Quantum Geometric Transformation (Rotation)
def quantum_rotate(image):
    return np.rot90(image)

# 3️⃣ Quantum Image Filtering (Low-pass smoothing)
...
# ======================================================
# 3❅ LOAD ONE RANDOM IMAGE (ROBUST)
# ======================================================
while True:
...
# ======================================================
# 5❅ BUILD IMAGE-SPECIFIC HD-QSAFE CIRCUIT
# ======================================================
...
# ======================================================
# 6❅ GENERATE HD-QSAFE QUANTUM IMAGE (PROBABILITY MAP)
# ======================================================
...
# ======================================================
# 5➔ QUANTUM IMAGE OPERATIONS (HD-QSAFE-AWARE)
# ======================================================

# ↔️ Quantum Rotation (90°)
...
# 💤 Quantum Filtering (Probability-domain smoothing)
...
def frqi_circuit(image):
    N = image.shape[0]
    n_pos = int(np.log2(N * N))
    total_qubits = n_pos + 1
    ...
    for idx, pixel in enumerate(flat_img):
        theta = 2 * np.arcsin(pixel)
        bin_state = format(idx, f'0{n_pos}b')

        for i, bit in enumerate(bin_state):
            if bit == '0':
                qc.x(i)

        qc.mcry(theta, list(range(n_pos)), n_pos)
...
def neqr_circuit(image):
    ...
    total_qubits = n_pos + n_color
    qc = QuantumCircuit(total_qubits)

    for i in range(n_pos):
        qc.h(i)

    flat_img = (image.flatten() * 255).astype(int)

    for idx, pixel in enumerate(flat_img):
        bin_pos = format(idx, f'0{n_pos}b')
        bin_color = format(pixel, '08b')

        for i, bit in enumerate(bin_pos):
            if bit == '0':
                qc.x(i)

        for j, bit in enumerate(bin_color):
            if bit == '1':
...
def compute_block_features(block):
    ...
    # Align shapes
    min_rows = min(gx.shape[0], gy.shape[0])
    min_cols = min(gx.shape[1], gy.shape[1])

    gx = gx[:min_rows, :min_cols]
    gy = gy[:min_rows, :min_cols]

    grad_mag = np.mean(np.sqrt(gx**2 + gy**2)) if gx.size > 0 else 0.0

    return np.array([mu, var, grad_mag])
...
def hierarchical_partition(image, block_size=2):
    blocks = []
    N = image.shape[0]
...
def hdqsafe_circuit(image, K=64, block_size=2):
    # Step 1: Partition
    blocks = hierarchical_partition(image, block_size)

    features = [] # Will store [mean, var, grad_mag] for each block
    scores = []

    # Step 2: Feature extraction
    for block in blocks:
        f = compute_block_features(block)
        features.append(f)
        scores.append(np.linalg.norm(f))

    features = np.array(features) # Shape: (num_blocks, 3)
    ...
    # Select only the 'mean' feature from the top K blocks, as done in hd_qsafe_encode
    selected_means = features[top_indices, 0] # Take the first feature (mean)

    # Convert to amplitudes: ensure positivity, take sqrt, apply gamma
    # Use global GAMMA and EPS from kernel state
    amplitudes = np.sqrt(selected_means + EPS)
    amplitudes = amplitudes ** GAMMA

    # Step 5: Normalize these amplitudes
    norm = np.linalg.norm(amplitudes)
    if norm == 0:
        norm = 1e-8 # Prevent division by zero
    amplitudes /= norm

    feature_vector = amplitudes # Now feature_vector is correctly formed amplitudes

    # Step 6: Determine qubit count
    # n_qubits must be based on the length of feature_vector, which is now 'amplitudes'
    n_qubits = int(np.ceil(np.log2(len(feature_vector))))

    qc = QuantumCircuit(n_qubits)

    state = np.zeros(2**n_qubits)
    state[:len(feature_vector)] = feature_vector
...
# ============================================================
# HD-QSAFE — FIXED hdqsafe_circuit + comparison loop
# Fix: strict L2-renormalisation before StatePreparation so
#      floating-point drift never causes the norm ≠ 1 error.
# ============================================================
...
def hdqsafe_circuit(image: np.ndarray,
                    blocks_per_dim: int = 2,
                    top_k: int = 16,
                    gamma: float = 0.8) -> QuantumCircuit:
    ...
    # FULL feature vector (μ, var, grad)
    selected = features[idx].flatten()

    # --- amplitude construction ---
    amplitudes = np.sqrt(np.abs(selected) + EPS) ** gamma
    ...
    n = int(np.ceil(np.log2(max(len(amplitudes), 2))))
    padded = np.zeros(2**n)
    padded[:len(amplitudes)] = amplitudes
    ...
    # --- build circuit ---
    qc = QuantumCircuit(n, name="HD-QSAFE")

    qc.h(range(n))

    for q in range(n):
        angle = gamma * np.pi * float(padded[q] + EPS)
        qc.ry(angle, q)

    qc.append(StatePreparation(padded, normalize=False), range(n))
    ...
    for q in range(n):
        angle = (1 - gamma) * np.pi * float(padded[(q * 3) % len(padded)] + EPS)
        qc.rz(angle, q)
...
# ============================================================
image_list = glob.glob(os.path.join("/content/converted_jpg_images", "*.jpg"))
print(f"Total images found: {len(image_list)}")

results = []

for img_path in image_list[:20]:
    try:
        image = load_image(img_path, size=8)

        frqi_c    = frqi_circuit(image)
        neqr_c    = neqr_circuit(image)
        hdqsafe_c = hdqsafe_circuit(image)       # ← uses fixed version

        results.append({
            "FRQI_qubits"  : frqi_c.num_qubits,
            "NEQR_qubits"  : neqr_c.num_qubits,
            "HD_qubits"    : hdqsafe_c.num_qubits,

            "FRQI_depth"   : frqi_c.depth(),
            "NEQR_depth"   : neqr_c.depth(),
            "HD_depth"     : hdqsafe_c.depth(),

            "FRQI_gates"   : frqi_c.size(),
            "NEQR_gates"   : neqr_c.size(),
            "HD_gates"     : hdqsafe_c.size(),
        })
...
print(f"\nSuccessfully processed: {len(df)} / 20 images")
...
models = ["FRQI", "NEQR", "HD_QSAFE"]

qubits = [
    mean_values["FRQI_qubits"],
    mean_values["NEQR_qubits"],
    mean_values["HD_qubits"]
]

depths = [
    mean_values["FRQI_depth"],
    mean_values["NEQR_depth"],
    mean_values["HD_depth"]
]

gates = [
    mean_values["FRQI_gates"],
    mean_values["NEQR_gates"],
    mean_values["HD_gates"]
]

plt.figure(figsize=(12,4))

plt.subplot(1,3,1)
plt.bar(models, qubits)
plt.title("Mean Qubits")

plt.subplot(1,3,2)
plt.bar(models, depths)
plt.title("Mean Circuit Depth")

plt.subplot(1,3,3)
plt.bar(models, gates)
plt.title("Mean Gate Count")
